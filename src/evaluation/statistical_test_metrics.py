...
data_file = open(path_output + 'statistical-tests-metrics-{}.csv'.format(data_name),
                      mode='w', newline='',
                      encoding='utf-8')
    ...
    for i in range(len(Feature_smote)):
        ...
        if feature in data_performace_metrics.keys():
            if not droped_attrib in data_performace_metrics[feature].keys():
                data_performace_metrics[feature][droped_attrib] = [acc_smote[i], acc_after_smote[i], Pre_smote[i],
                                                               Pre_after_smote[i], Re_smote[i], Re_after_smote[i],
    ...
    for key, val in data_fairness_metrics.items():
        ...
        for key2, val2 in val.items():
            if key != 'None':
                ...
                # corr, _ = pearsonr(val_before, val_after)
                #
