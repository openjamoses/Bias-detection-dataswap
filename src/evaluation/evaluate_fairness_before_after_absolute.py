...
data_file_fairness = open(path_output + '/Fairnmess-metrics-Baseline-after-{}-v2.csv'.format(naming), mode='w',
                          newline='',
                          encoding='utf-8')
...
for baseline_ in BASELINES:
    ...
    FPR_diff_smote = [abs(a) for a in group_data.FPR_diff.values.tolist()]
    FPR_diff_after_smote = [abs(a) for a in group_data.FPR_diff_after.values.tolist()]
    SPD_smote = [abs(a) for a in group_data.SPD.values.tolist()]
    SPD_after_smote = [abs(a) for a in group_data.SPD_after.values.tolist()]
    DIR_smote = group_data.DIR.values.tolist()
    DIR_after_smote = group_data.DIR_after.values.tolist()
    AOD_smote = [abs(a) for a in group_data.AOD.values.tolist()]
    AOD_after_smote = [abs(a) for a in group_data.AOD_after.values.tolist()]
    ...
    for i in range(len(Feature_smote)):
        ...
        fpr, fpr_diff, spd, aod, dir = FPR_after_smote[i], FPR_diff_after_smote[i], SPD_after_smote[i], AOD_after_smote[
            i], DIR_after_smote[i]
        ...
        ranking_22 = fpr + fpr_diff + spd + aod + dir
        ranking_02 = FPR_smote[i] + FPR_diff_smote[i] + SPD_smote[i] + AOD_smote[i] + DIR_smote[i]
        # if most_min <= 0:
        fpr, fpr_diff, spd, aod, dir = fpr, fpr_diff, spd, aod, dir
        # else:
        ...
        dist_default = [FPR_diff_smote[i], SPD_smote[i], DIR_smote[i], AOD_smote[i], acc_smote[i], Pre_smote[i],
                        Re_smote[i], F1_smote[i]]
        dist_after = [FPR_diff_after_smote[i], SPD_after_smote[i], DIR_after_smote[i], AOD_after_smote[i],
                      acc_after_smote[i], Pre_after_smote[i],
        ...
        data_writer_fairness.writerow([Feature_smote[i], Droped_Attrib[i], baseline_, round(FPR_diff_smote[i], dp),
                                       round(FPR_diff_after_smote[i], dp),
                                       round(FPR_diff_after_smote[i] - FPR_diff_smote[i], dp),
                                       round(SPD_smote[i], dp), round(SPD_after_smote[i], dp),
                                       round(SPD_after_smote[i] - SPD_smote[i], dp), round(DIR_smote[i], dp),
                                       round(DIR_after_smote[i], dp), round(DIR_after_smote[i] - DIR_smote[i], dp),
                                       round(AOD_smote[i], dp), round(AOD_after_smote[i], dp),
                                       round(AOD_after_smote[i] - AOD_smote[i], dp), round(FPR_smote[i], dp),
                                       round(FPR_after_smote[i], dp), round(FPR_after_smote[i] - FPR_smote[i], dp),
                                       round(SP_smote[i], dp), round(SP_after_smote[i], dp),
                                       round(SP_after_smote[i] - SP_smote[i], dp), round(acc_smote[i] * 100, 2),
                                       round(acc_after_smote[i] * 100, 2), round(acc_after_smote[i] - acc_smote[i], dp),
                                       round(Pre_smote[i] * 100, 2), round(Pre_after_smote[i] * 100, 2),
                                       round(Pre_after_smote[i] - Pre_smote[i], dp), round(Re_smote[i] * 100, 2),
                                       round(Re_after_smote[i] * 100, 2), round(Re_after_smote[i] - Re_smote[i], dp),
                                       round(F1_smote[i] * 100, 2), round(F1_after_smote[i] * 100, 2),
                                       round(F1_after_smote[i] - F1_smote[i], dp), ranking_1, ranking_2, ranking,
                                       ranking_, ranking_before,
                                       p_value, delta])
