...
data_file2 = open(path_output + '/Natural-impact/Confounding_Others_features_{}-revision-50.csv'.format(data_name),
                  mode='w', newline='',
                  encoding='utf-8')
data_writer2 = csv.writer(data_file2)
data_writer2.writerow(
    ['Feature', 'Swap \n Percentage', 'Metric', 'CDI', 'NDI', 'NII', 'Impact_Sum', 'AVERAGE', 'NDI2', 'NII2'])
data_file3 = open(path_output + '/Natural-impact/Correlated_features_{}-revision-50.csv'.format(data_name), mode='w',
                  newline='',
                  encoding='utf-8')
...
feature_list2_ = set(Feature2)
data_feature = {}
for i in range(len(Feature)):
    if Feature[i] in data_feature.keys():
        data_feature[Feature[i]] += 1
    else:
        data_feature[Feature[i]] = 1
feature_list2 = []
feature_list = []
data_feature = sort_dict(data_feature)
for k in data_feature.keys():
    feature_list2.append(k)
    feature_list.append(k)
for f in feature_list2_:
    if not f in feature_list2:
    ...
    if feature_ in data_probability_mapping_CDI.keys():
        ...
        for swap_prop, val2_ in val.items():
            if swap_prop in swap_selected:
                row_hellinger = ['{}%'.format(swap_prop), 'Hellinger distance', feature_]
                row_wasserstein = ['{}%'.format(swap_prop), 'Wasserstein distance', feature_]
                row_total_variation = ['{}%'.format(swap_prop), 'Total variation distance', feature_]
                row_js = ['{}%'.format(swap_prop), 'Jensen-Shannon divergence', feature_]
                row_Casuality = ['{}%'.format(swap_prop), 'Propensity score', feature_]
                    ...
                    if feature_ + '/' + feature_2 in data_probability_mapping_NDI.keys():
                        row_hellinger.append(
                            round(data_probability_mapping_NDI[feature_ + '/' + feature_2][swap_prop]['hellinger'],
                                  2) +
                            round(data_probability_mapping_NII[feature_ + '/' + feature_2][swap_prop]['hellinger'], 2))
                        ...
                        row_js.append(
                            round(data_probability_mapping_NDI[feature_ + '/' + feature_2][swap_prop]['JS'],
                                  2) +
                            round(data_probability_mapping_NII[feature_ + '/' + feature_2][swap_prop]['JS'], 2))

                        row_Casuality.append(
                            round(data_probability_mapping_NDI[feature_ + '/' + feature_2][swap_prop]['Casuality'],
                                  2) +
                            round(data_probability_mapping_NII[feature_ + '/' + feature_2][swap_prop]['Casuality'], 2))
                ...
                data_writer.writerow(
                    [feature_, '{}%'.format(swap_prop), 'Hellinger distance(+)',
                ...
                data_writer.writerow(
                    [feature_, '{}%'.format(swap_prop), 'Wasserstein distance(+)',
                ...
                data_writer.writerow(
                    [feature_, '{}%'.format(swap_prop), 'Total variation distance(+)',
                ...
                data_writer.writerow(
                    [feature_, '{}%'.format(swap_prop), 'Jensen–Shannon divergence(+)',
