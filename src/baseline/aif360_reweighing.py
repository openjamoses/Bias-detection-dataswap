...
class AIF360Reweigh:
    ...
    def fit_baseline(self, clf, p_pred, fold, droped_attrib, data_writer_baseline):
        ...
        # #x_columns = [self.colums_list[i] for i in range(self.train.shape[1]-1)]
        # X_train = pd.DataFrame(self.x_train, columns=x_columns)
        # X_test = pd.DataFrame(self.x_test, columns=x_columns)
        ...
        for column_id in range(self.x_train.shape[1]):
            ...
            print(self.colums_list[self.target_index], data_train_fm.columns)

            train_std = StandardDataset(data_train_fm_copy1,
            ...
            RW.fit(train_std)
            transf_train = RW.transform(train_std)

            X_train, y_train = transf_train.features, transf_train.labels.ravel()
